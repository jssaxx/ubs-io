...
static int32_t CmClientLocalCheckNode(uint16_t poolId, PoolInfo *poolInfo, NodeInfo *nodeInfo)
{
    ...
    if (nodeInfo->diskList.type != poolInfo->type) {
        CM_LOGERROR("Check node media failed, node(%u) config(%u).", nodeInfo->diskList.type, poolInfo->type);
        return CM_ERR;
    }
...
static int32_t CmClientLocalRegister(uint16_t poolId)
{
    NodeInfo *nodeInfo = &g_localNodeInfo[poolId];
    int32_t ret;
...
int32_t CmClientLocalInit(void)
{
    ...
    for (poolId = 0; poolId < MAX_POOL_NUM; poolId++) {
        ...
        ret = CmClientLocalRegister(poolId);
        if (ret != CM_OK) {
