...
void BioConfig::LoadDefaultConf()
{
    ...
    AddIntConf(NET_DATA_CTRL_WORKERS_COUNT, VIntRange::Create(NET_DATA_CTRL_WORKERS_COUNT.first, NO_1, NO_16));
    AddIntConf(NET_DATA_DATA_WORKERS_COUNT, VIntRange::Create(NET_DATA_DATA_WORKERS_COUNT.first, NO_1, NO_16));
    /* don't allow empty */
    ...
    /* don't allow empty */
    AddStrConf(NET_CMM_IP_MASK, VIpv4MaskValidator::Create(NET_CMM_IP_MASK.first, false));
    AddIntConf(NET_CMM_PORT, VIntRange::Create(NET_CMM_PORT.first, 0, NO_65535));
    ...
    AddIntConf(CM_ZK_HOST_PORT, VIntRange::Create(CM_ZK_HOST_PORT.first, NO_2048, NO_65535));
}
...
BResult BioConfig::AutoConfigNet(const ConfigurationPtr &conf)
{
    ...
    mNetConfig.cmmIpMask = conf->GetStr(NET_CMM_IP_MASK.first);
    mNetConfig.cmmPort = conf->GetInt(NET_CMM_PORT.first);

    /* fetch ip from ip mask, example:x.x.x.x/24 to x.x.x.x */
    std::vector<std::string> ipMaskVec;
    StrUtil::Split(mNetConfig.cmmIpMask, "/", ipMaskVec);
    if (ipMaskVec.size() != 2L) {
        LOG_ERROR("Failed to spilt ip with ip mask " << mNetConfig.cmmIpMask);
        return BIO_ERR;
    }
    mNetConfig.cmmIp = std::move(ipMaskVec[0]);
    mNetConfig.isBusyLoop = conf->GetBool(NET_DATA_BUSY_POLL_MODE.first);
    mNetConfig.ctrlWorkersCnt = conf->GetInt(NET_DATA_CTRL_WORKERS_COUNT.first);
    mNetConfig.dataWorkersCnt = conf->GetInt(NET_DATA_DATA_WORKERS_COUNT.first);
...
{
    mCmConfig.enableCmm = conf->GetBool(CMM_ENABLED.first);
    ...
    std::string zkHostIpMask = conf->GetStr(CM_ZK_HOST_IP_MASK.first);
    std::vector<std::string> ipMaskVec;
    StrUtil::Split(zkHostIpMask, "/", ipMaskVec);
    if (ipMaskVec.size() != 2L) {
        LOG_ERROR("Failed to spilt ip with ip mask " << zkHostIpMask);
        return BIO_ERR;
    }
    mCmConfig.zkHostIp = std::move(ipMaskVec[0]);
    mCmConfig.zkHostPort = conf->GetInt(CM_ZK_HOST_PORT.first);
