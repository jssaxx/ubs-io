...
#define TRACEPOINT_NOT_REGISTERED (1)
#define TRACEPOINT_MODID 381
...
{                                                                               \
    ...
    {                                                                           \
        ...
        {                                                                       \
            ...
            {                                                                   \
                _tracepointNotRegistered = TRACEPOINT_NOT_REGISTERED;           \
...
/** @ingroup VOS_TRACEP  
    ...
    @使用约束：支持激活次数的并发操作，有些并发场景不支持，如IO路径的并发场景，
                因为不同的并发场景需要不同的策略去防止并发，所以不能做到全场景防护。
                在不同的并发场景下，需要使用者做并发防护
    @param tracep_name [in] TracePoint名称，取值：长度小于128字节  
...
#define LVOS_TP_START(name, ...)                                                              \
          ...
          {                                                                                   \
            ...
            if (_ret >= 0 && _pstTp->type != LVOS_TP_TYPE_PAUSE) {                            \
                ...
                else if (_pstTp->type == LVOS_TP_TYPE_ABORT)                                  \
                {                                                                             \
                    raise(9);                                                                 \
                }                                                                             \
                else if (_pstTp->type == LVOS_TP_TYPE_RESET)                                  \
                {                                                                             \
                    dpax_Reboot((unsigned long)BSP_SOFT_REBOOT, TRACEPOINT_MODID, (char *)"tracepoint reboot");    \
                }                                                                             \
                if(_ret == 0) {                                                               \
...
/**
  @brief  初始化tracepoint模块，初始化内容包括TP点框架以及TP点相关的diagnose命令
  @verbatim   
   目标用户: 用户态APP
   使用约束: NA
   升级影响: NA
...
/**
  @brief  卸载tracepoint模块，卸载内容包括TP点框架以及TP点相关的diagnose命令
  @verbatim   
   目标用户: 用户态APP
   使用约束: NA
   升级影响: NA
