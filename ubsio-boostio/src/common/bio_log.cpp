...
int32_t Logger::Init()
{
    ...
    try {
        if (mOptions.logType == STDOUT_TYPE) { // stdout
            ...
            handlers.after_open = [this](const spdlog::filename_t &filename, std::FILE *fstream) {
                chmod(filename.c_str(), S_IRUSR | S_IWUSR | S_IRGRP);
                for (int i = 1; i <= mOptions.rotationFileCount; ++i) {
                    std::string backupName = filename + "." + std::to_string(i);
                    struct stat buffer{};
                    if (stat(backupName.c_str(), &buffer) == 0) {
                        chmod(backupName.c_str(), S_IRUSR | S_IRGRP);
                    }
                }
            };
