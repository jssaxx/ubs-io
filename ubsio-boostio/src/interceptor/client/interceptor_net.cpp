...
void InterceptorClientNetService::OrphanInheritedState(uint32_t currentPid, bool needLog)
{
    if (needLog) {
    ...
    if (mDataMsgMemPool != nullptr) {
        mDataMsgMemPool->IncreaseRef();
        mDataMsgMemPool = nullptr;
    }
    if (mNetEngine != nullptr) {
        mNetEngine->IncreaseRef();
        mNetEngine = nullptr;
    }
...
BResult InterceptorClientNetService::PrepareAfterForkChild()
{
    ...
    if (mPid != 0 && mPid != currentPid) {
        OrphanInheritedState(currentPid, false);
    }
    return BIO_OK;
}
...
void InterceptorClientNetService::AbandonNetService()
{
    mReady.store(false);
