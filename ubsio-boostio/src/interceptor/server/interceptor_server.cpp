...
BResult InterceptorServer::RegisterOpcode()
{
    ...
    auto ret = netEngine->RegisterNewRequestHandler(
        BIO_OP_INTERCEPTOR_READ, std::bind(&InterceptorServer::HandleInterceptorRead, this, std::placeholders::_1));
    if (ret != BIO_OK) {
        CLIENT_LOG_ERROR("Register interceptor read message handle failed, ret:" << ret << ".");
        return ret;
    }
    ret = netEngine->RegisterNewRequestHandler(
        BIO_OP_INTERCEPTOR_WRITE, std::bind(&InterceptorServer::HandleInterceptorWrite, this, std::placeholders::_1));
    if (ret != BIO_OK) {
...
BResult InterceptorServer::HandleInterceptorRead(ServiceContext &ctx)
{
    ...
    CLIENT_LOG_DEBUG("Receive interceptor read message inode:" << req->inode << " offset:" << req->offset
    ...
    if (UNLIKELY(resp == nullptr)) {
        CLIENT_LOG_ERROR("Alloc memory failed, inode:" << req->inode << " offset:" << req->offset
                                                       << " len:" << req->nbytes << " fd:" << req->fd << ".");
    ...
    if (UNLIKELY(ret != 0)) {
        CLIENT_LOG_ERROR("Read hook failed, inode:" << req->inode << " offset:" << req->offset << " len:" << req->nbytes
                                                    << " fd:" << req->fd << ", readLen:" << readLen << ", ret:" << ret
                                                    << ".");
        BioClientNet::Instance()->GetNetEngine()->Reply(ctx, BIO_ALLOC_FAIL, nullptr, 0);
        BIO_TRACE_END(MIRROR_TRACE_INTERCEPTOR_READ, BIO_ALLOC_FAIL);
        free(resp);
...
BResult InterceptorServer::HandleInterceptorWrite(ServiceContext &ctx)
{
    ...
    CLIENT_LOG_DEBUG("Receive interceptor write message inode:" << req->inode << " offset:" << req->offset
                                                                << " len:" << req->nbytes << " fd:" << req->fd);
...
BResult InterceptorServer::HandleInterceptorAllocPage(ServiceContext &ctx)
{
    ...
    CLIENT_LOG_DEBUG("Alloc put value with space length:" << req->length
                                                          << ", location0:" << addressInfo.loc.location[0]
                                                          << ", location1:" << addressInfo.loc.location[1]
                                                          << ", address0 size:" << addressInfo.address[0].size
                                                          << ", address1 size:" << addressInfo.address[1].size
                                                          << ", address num:" << addressInfo.addressNum << ".");
...
BResult InterceptorServer::HandleInterceptorLargeWrite(ServiceContext &ctx)
{
    ...
    CLIENT_LOG_DEBUG("Alloc put value with space length:" << req->nbytes
                                                          << ", location0:" << addressInfo.loc.location[0]
                                                          << ", location1:" << addressInfo.loc.location[1]
                                                          << ", address0 size:" << addressInfo.address[0].size
                                                          << ", address1 size:" << addressInfo.address[1].size << ".");
