...
#define LOAD_SYM_OR_RETURN(handle, sym, ptr)                               \
    do {                                                                   \
        if (LoadSym(handle, sym, reinterpret_cast<void**>(&(ptr))) != 0) { \
            return -1;                                                     \
        }                                                                  \
    } while (0)

#define LOAD_SYM_FALLBACK_OR_RETURN(handle, sym1, sym2, ptr)                                  \
    do {                                                                                      \
        if (LoadSymWithFallback(handle, sym1, sym2, reinterpret_cast<void**>(&(ptr))) != 0) { \
            return -1;                                                                        \
        }                                                                                     \
    } while (0)
