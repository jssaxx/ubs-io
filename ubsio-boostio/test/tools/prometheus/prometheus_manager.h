...
private:
    ...
    void GetRemoteTracePoints(std::vector<uint16_t> nodeIds, std::map<uint16_t, TraceDatabase> &nodesTracePoints);
    ...
    inline void UpdateCacheHitRatio(std::map<uint16_t, prometheus::Gauge *> &dataMap,
                                    prometheus::Family<prometheus::Gauge> *dataFamily, uint16_t key, double value)
    {
    ...
    inline void UpdateFaultNodeCacheHitRatio(std::map<uint16_t, prometheus::Gauge *> &dataMap,
                                             prometheus::Family<prometheus::Gauge> *dataFamily, uint16_t faultNodeId)
    {
        std::lock_guard<std::mutex> lock(mMutex);
    ...
    inline void UpdateCacheMetric(std::map<uint16_t, prometheus::Gauge *> &cacheMetrics,
                                  prometheus::Family<prometheus::Gauge> *cacheMetricFamily, uint16_t nodeId,
                                  double value, bool isMB = false)
    {
    ...
    inline void UpdateTraceMetrics(
        std::unordered_map<uint16_t, std::unordered_map<std::string, prometheus::Gauge *>> &traceMaps,
        prometheus::Family<prometheus::Gauge> *traceFamily, uint16_t nodeId, std::string traceName, double value)
    {
    ...
    inline void UpdateFaultNodeTraceMetrics(
        std::unordered_map<uint16_t, std::unordered_map<std::string, prometheus::Gauge *>> &traceMaps,
        prometheus::Family<prometheus::Gauge> *traceFamily, uint16_t nodeId)
    {
        auto it = traceMaps.find(nodeId);
        if (it == traceMaps.end()) {
            return;
    ...
    inline prometheus::Family<prometheus::Gauge> *RegisterGauge(
        prometheus::Registry &registry, const std::string &name, const std::string &help,
        const std::initializer_list<std::pair<std::string, std::string>> &labels = {})
    {
        auto gauge = &prometheus::BuildGauge().Name(name).Help(help).Register(registry);
        if (!gauge) {
    ...
    {
        mTotalCacheHitRatioFamily = RegisterGauge(*mPrometheusRegistry, "total_hit_ratio", "total hit ratio");
        mRCacheHitRatioFamily = RegisterGauge(*mPrometheusRegistry, "read_cache_hit_ratio", "read cache hit ratio");
        mRCacheHitMemRatioFamily =
            RegisterGauge(*mPrometheusRegistry, "read_mem_cache_hit_ratio", "read memory cache hit ratio");
        mRCacheHitDiskRatioFamily =
            RegisterGauge(*mPrometheusRegistry, "read_disk_cache_hit_ratio", "read disk cache hit ratio");
        mWCacheHitRatioFamily = RegisterGauge(*mPrometheusRegistry, "write_cache_hit_ratio", "write cache hit ratio");
        mWCacheHitMemRatioFamily =
            RegisterGauge(*mPrometheusRegistry, "write_mem_cache_hit_ratio", "write memory cache hit ratio");
        mWCacheHitDiskRatioFamily =
            RegisterGauge(*mPrometheusRegistry, "write_disk_cache_hit_ratio", "write disk cache hit ratio");
        mBackendHitRatioFamily = RegisterGauge(*mPrometheusRegistry, "backend_hit_ratio", "backend hit ratio");
    }

    inline void RegisterWCacheMetrics()
    {
        mWCacheMemWaterLevelFamily =
            RegisterGauge(*mPrometheusRegistry, "wcache_mem_water_level", "Write Cache Memory Water Level");
        mWCacheMemCapacityFamily =
            RegisterGauge(*mPrometheusRegistry, "wcache_mem_capacity", "Write Cache Memory Capacity");
        mWCacheMemUsedSizeFamily =
            RegisterGauge(*mPrometheusRegistry, "wcache_mem_used_size", "Write Cache Memory Used Size");
        mWCacheDiskWaterLevelFamily =
            RegisterGauge(*mPrometheusRegistry, "wcache_disk_water_level", "Write Cache Disk Water Level");
        mWCacheDiskCapacityFamily =
            RegisterGauge(*mPrometheusRegistry, "wcache_disk_capacity", "Write Cache Disk Capacity");
        mWCacheDiskUsedSizeFamily =
            RegisterGauge(*mPrometheusRegistry, "wcache_disk_used_size", "Write Cache Disk Used Size");
    }

    inline void RegisterRCacheMetrics()
    {
        mRCacheMemCapacityFamily =
            RegisterGauge(*mPrometheusRegistry, "rcache_mem_capacity", "Read Cache Memory Capacity");
        mRCacheMemUsedSizeFamily =
            RegisterGauge(*mPrometheusRegistry, "rcache_mem_used_size", "Read Cache Memory Used Size");
        mRCacheMemWaterLevelFamily =
            RegisterGauge(*mPrometheusRegistry, "rcache_mem_water_level", "Read Cache Memory Water Level");
        mRCacheDiskCapacityFamily =
            RegisterGauge(*mPrometheusRegistry, "rcache_disk_capacity", "Read Cache Memory Capacity");
        mRCacheDiskUsedSizeFamily =
            RegisterGauge(*mPrometheusRegistry, "rcache_disk_used_size", "Read Cache Memory Used Size");
        mRCacheDiskWaterLevelFamily =
            RegisterGauge(*mPrometheusRegistry, "rcache_disk_water_level", "Read Cache Memory Water Level");
    }
