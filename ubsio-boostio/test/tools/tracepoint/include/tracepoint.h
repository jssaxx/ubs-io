...
#define BIO_TRACEP_DEF0(tracep_name) typedef void (*FN_TRACEP_T_##tracep_name)(BioTracepointParam *);

#define BIO_TRACEP_DEFN(tracep_name, ...) typedef void (*FN_TRACEP_T_##tracep_name)(BioTracepointParam *, __VA_ARGS__);
...
#define BIO_TRACEP_CALL(tracep_name, ...)                                                                          \
    do {                                                                                                           \
        ...
        for (_i = 0; _i < BIO_MAX_HOOK_PER_TRACEP; _i++) {                                                         \
            if ((_pstTp->stHooks[_i].iActive == BIO_TRACEP_STAT_ACTIVE) && (NULL != _pstTp->stHooks[_i].fnHook)) { \
...
#define BIO_TRACEPHOOK_CALL(tracep_name, fn, ...)                                                  \
    do {                                                                                           \
        ...
        if ((_pstHook->iActive == BIO_TRACEP_STAT_ACTIVE) && (NULL != _pstHook->fnHook)) {         \
