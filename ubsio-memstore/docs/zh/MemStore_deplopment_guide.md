...
3. 参照表2修改所有节点的配置文件/etc/mms/mms.conf。
    ...
    **表 2**  MMS配置项
    ...
    |mms.net.rpc.busy_polling_mode|rpc通信的工作方式是否是busy polling。|ture|truefalse|
    ...
    |mms.net.ipc.busy_polling_mode|ipc通信的工作方式是否是busy polling。|true|truefalse|
    ...
    |mms.cm.zk_host|集群Zookeerper地址。|127.0.0.1:2181|*.*.*.*:#，*为0 ~ 255，#为0 ~ 65535|
