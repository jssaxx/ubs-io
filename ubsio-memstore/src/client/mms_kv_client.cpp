...
static BResult FillOneValueInfoItem(SearchValueInfoCtx &ctx, uint64_t index)
{
    ValueInfo &item = ctx.valueInfoItems[index];
    ValueDesInfo &des = ctx.des->values[index];
    item.key = ctx.memAddr + ctx.memOffset;
    BResult ret = AdvanceSearchResultOffset(ctx.totalSize, des.keyLen, ctx.memOffset);
    if (UNLIKELY(ret != MMS_OK)) {
        CLIENT_LOG_ERROR("Search result key length invalid, item index:" << index << ".");
        return ret;
    }
    ...
    if (UNLIKELY(ret != MMS_OK)) {
        CLIENT_LOG_ERROR("Search result value length invalid, item index:" << index << ".");
        return ret;
    }
    return MMS_OK;
}
                                        ...
                                        uint64_t *itemNum)
{
    ...
    if (UNLIKELY(rsp.totalSize < sizeof(PrefixSearchDes) || des->totalSize != rsp.totalSize ||
        ...
        CLIENT_LOG_ERROR("Search result header invalid, total size:" << rsp.totalSize << ".");
    ...
    *itemNum = des->itemNum;
    ...
    if (UNLIKELY(ret != MMS_OK)) {
        CLIENT_LOG_ERROR("Search result offset invalid, item num:" << des->itemNum
                                                                   << ", total size:" << rsp.totalSize << ".");
        UnmapSearchResult(dataPtr, rsp.totalSize);
        return ret;
    }
    ...
    *valueInfoItems = new (std::nothrow) ValueInfo[des->itemNum];
    ...
    SearchValueInfoCtx searchCtx = {memAddr, rsp.totalSize, memOffset, des, *valueInfoItems};
    for (uint64_t index = 0; index < des->itemNum; index++) {
        ret = FillOneValueInfoItem(searchCtx, index);
        if (UNLIKELY(ret != MMS_OK)) {
