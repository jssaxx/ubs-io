...
public:
   ...
   BResult GetPtInfo(uint16_t &ptId, uint64_t &ptv, std::unordered_set<std::string> &remoteIps)
   {
       ReadLocker<ReadWriteLock> lock(&mPLock);
       if (mPtInfo.state != CM_PT_NORMAL) {
           LOG_WARN("Not ready, ptId:" << mPtInfo.ptId << ", state:" << mPtInfo.state);
       ...
       for (const auto& elem : mPtInfo.copys) {
           ...
           if (elem.nodeId != mNodeId) {
               ReadLocker<ReadWriteLock> nLock(&mNLock);
               auto it = mNodeInfos.find(elem.nodeId);
               if (it == mNodeInfos.end()) {
                   continue;
               }
               remoteIps.emplace(it->second.ip);
           }
       }
       return MMS_OK;
   }
